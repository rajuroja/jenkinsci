...
public class CaseRegistrationController {
	...
	private static final Logger LOGGER = LogManager.getLogger(CaseRegistrationController.class);

	@PostMapping(value="/saveCaseDetails" , consumes = { MediaType.APPLICATION_JSON_VALUE, MediaType.MULTIPART_FORM_DATA_VALUE })
	public ApiResponse<Object> saveCaseWithLogin(@Valid @RequestPart("data") CaseDetailsRequest caseDetailsRequest,@RequestPart("filelist") List<MultipartFile> caseFiles){
		try {
	...
	@PostMapping("/saveCaseInquiryDetails")
	public ApiResponse<Object> saveCaseInquiryDetails(@RequestBody CaseDetailsRequest caseDetailsRequest){
		try {
				return new ApiResponse<>(true, HttpStatus.OK.name(), "Success", "Case registred Successfully");
		} catch (DataIntegrityViolationException e) {
			LOGGER.error("Error during saving case due to data integrity violation exception error ");
